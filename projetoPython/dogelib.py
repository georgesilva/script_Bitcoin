...
def openBrowser(wallet, password):
	...
	# Open some site
	response = br.open('http://freedoge.co.in/')


	#Doing the login
	br.select_form(nr=2)
	br['doge_address'] = wallet
	br['password'] = password
	response = br.submit()
	httpResult = response.read().split(":")

	#Setting cookies
	cookieDOGEAddress = "doge_address="+httpResult[1]+"; expires=3650"
	cookiePassword = "password="+httpResult[2]+"; expires=3650"
	br.set_cookie(cookieDOGEAddress)
	...
	response = br.open('http://freedoge.co.in/?op=home&tab=double_your_doge#')
...
def makeBet(br, initBet, stopPercentage, odds, multiplier, imfe):
	"Funcao que faz a aposta utilizando os parametros. Retorna o balanço atualizado em DOGEs."
	global iterator
	global saldoInicial
	global stop
	global sumLosses
	...
	with GracefulInterruptHandler() as h:
		while True:
			...
			signal.alarm(25)
			try:
				response = br.open('http://freedoge.co.in/cgi-bin/bet.pl?m='+mode+'&client_seed='+clientSeed+'&jackpot=0&stake='+
...
def getClientSeed(br):
	...
	response = br.open('https://freedoge.co.in/?op=home&tab=double_your_doge#')
